...
const proxy = (req: Request, res: Response, next: NextFunction) => {
    ...
    let reqPath = path.join(__dirname, '../deskpassServices/'); // get the path to each requested services 
    ...
    if (deskpassApprovedFetchSites.includes(requestURL)) {
        ...
        res.sendFile(`${requestService}.html`, { root: reqPath }); // route to the service path/address if found
    } else {
