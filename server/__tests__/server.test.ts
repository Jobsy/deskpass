...
describe("GET / - deskpass endpoints - /deskpass/cocktail ", () => {
  ...
  it("/name API Request", async () => {
    const result = await request(app).get("/deskpass/cocktail/name/?is18=true&alcoholic=false");
    
    expect(result.statusCode).toEqual(200);
  })

  it("/random API Request", async () => {
    const result = await request(app).get("/deskpass/cocktail/random/?is18=true&alcoholic=false");
...
describe("GET / - deskpass endpoints - /deskpass/country API Request", () => {
  ...
  it("/name", async () => {
    const result = await request(app).get("/deskpass/country/name/?country=nigeria");
    expect(result.statusCode).toEqual(200);
  });
})
